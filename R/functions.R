...
results_gene <- dataset %>% filter(!is.na(Gene))
  ...
  ggplot(results_gene, 
          ...
          labs(title = file) +
          geom_point() +
          geom_errorbar(aes(ymin = Mean.Ct - Std.Dev..Ct, ymax=Mean.Ct+Std.Dev..Ct), 
                        width=.2,
...
# Regression analysis, visualization and calculation of efficiency --------
...
results <- results_gene %>% filter(Sample.name %in% c("Std01", "Std02","Std03", 
                                                      "Std04","Std05", "Std06", 
                                                      "Std07", "Std08"))
...
# Ct_mean = slope*(log(Qty)) + intercept
  ...
  label_r <- paste0(label_r, as.character(format(
    summary(regression_mean)$r.squared), digit = 3))
  ...
  ggplot(results, 
    ...
    geom_errorbar(aes(ymin = Mean.Ct - Std.Dev..Ct, 
                      ymax = Mean.Ct + Std.Dev..Ct), width = .2,
